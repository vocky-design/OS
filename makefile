...
LIB = -I lib/ -I lib/kernel/ -I kernel/ -I kernel/thread/ -I kernel/device/
ASFLAGS = -f elf32
...
####################### C编译 #######################################
##KERNEL##
$(BUILD_DIR)/main.o: kernel/main.c lib/kernel/stdint.h lib/kernel/print.h kernel/debug.h kernel/init.h  kernel/interrupt.h  kernel/device/console.h kernel/memory.h kernel/thread/thread.h kernel/device/console.h
	$(CC) $(CFLAGS) -o $@ $<
$(BUILD_DIR)/init.o: kernel/init.c kernel/init.h lib/kernel/print.h kernel/interrupt.h kernel/timer.h kernel/memory.h kernel/thread/thread.h kernel/device/console.h kernel/device/keyboard.h
	$(CC) $(CFLAGS) -o $@ $<
$(BUILD_DIR)/interrupt.o: kernel/interrupt.c kernel/interrupt.h lib/kernel/print.h lib/kernel/stdint.h kernel/global.h lib/kernel/io.h 
	$(CC) $(CFLAGS) -o $@ $<
$(BUILD_DIR)/timer.o: kernel/timer.c kernel/timer.h lib/kernel/io.h lib/kernel/print.h kernel/debug.h kernel/thread/thread.h kernel/interrupt.h
	$(CC) $(CFLAGS) -o $@ $<
...
##KERNEL/THREAD##
$(BUILD_DIR)/thread.o: kernel/thread/thread.c kernel/thread/thread.h lib/kernel/stdint.h lib/string.h kernel/memory.h kernel/interrupt.h lib/kernel/list.h kernel/debug.h lib/kernel/list.h lib/kernel/print.h
	$(CC) $(CFLAGS) -o $@ $<
$(BUILD_DIR)/sync.o: kernel/thread/sync.c kernel/thread/sync.h lib/kernel/stdint.h lib/kernel/list.h kernel/debug.h kernel/interrupt.h kernel/thread/thread.h
	$(CC) $(CFLAGS) -o $@ $<
##KERNEL/DEVICE##
...
$(BUILD_DIR)/keyboard.o: kernel/device/keyboard.c kernel/device/keyboard.h lib/kernel/stdint.h lib/kernel/print.h lib/kernel/io.h kernel/interrupt.h kernel/device/ioqueue.h
	$(CC) $(CFLAGS) -o $@ $<
$(BUILD_DIR)/ioqueue.o: kernel/device/ioqueue.c	kernel/device/ioqueue.h	lib/kernel/stdint.h kernel/thread/thread.h kernel/thread/sync.h kernel/debug.h kernel/interrupt.h
	$(CC) $(CFLAGS) -o $@ $<
